...
- **✅ Analizador — extras (2026-07-17):** (1) **límite 200 palabras** en Trade Construction (contador en vivo cliente + corte server-side en `/analyze`). (2) **switch de idioma del análisis ya arrojado** — chips EN/ES/FR/PT sobre el resultado → endpoint `/translate` (texto-only, cachea por idioma cliente-side, traduce SIEMPRE desde el original). Usa `TRANSLATE_MODEL` (env, default `gpt-4o-mini`, ~$0.0003/switch); `record_ai_cost` ahora es model-aware (`AI_PRICES` mapea precios por modelo → panel exacto). Claves i18n `result.langLabel/translating/translateErr`, `notes.words`.
