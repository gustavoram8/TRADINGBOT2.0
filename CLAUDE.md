...
- **✅ Analizador — extras (2026-07-17):** **límite 200 palabras** en Trade Construction (contador en vivo `#notes-counter`/`#notes-count` cliente, se pone rojo y recorta el paste; + corte server-side en `/analyze`, clave i18n `notes.words`). **NOTA:** un switch de idioma del análisis arrojado (endpoint `/translate` + chips EN/ES/FR/PT) se construyó y luego se **RETIRÓ por decisión del usuario** (evitar cobros de más; el trader ya tiene su idioma preseteado antes de analizar) — no re-agregar salvo pedido explícito.
